...
public class EconomyTradeData extends SavedData {
    ...
    private static final SavedDataType<EconomyTradeData> TYPE = new SavedDataType<>(
            Identifier.withDefaultNamespace(NAME),
            level -> new EconomyTradeData(),
            level -> CompoundTag.CODEC.xmap(
                    EconomyTradeData::load,
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            try {
                data.trades.add(new TradeSnapshot(
                    ...
                    t.contains("CommodityType") ? t.getStringOr("CommodityType", "") : null,
                    t.getStringOr("Price", ""),
                    t.getIntOr("Quantity", 0),
                    com.nstut.economy.util.NbtCompat.getUuid(t, "Buyer"),
                    com.nstut.economy.util.NbtCompat.getUuid(t, "Seller"),
                    t.getLongOr("Timestamp", 0L)
                ));
            } catch (Exception e) {
            }
        }
    ...
    public CompoundTag save(CompoundTag tag) {
        ...
        for (TradeSnapshot t : trades) {
            ...
            com.nstut.economy.util.NbtCompat.putUuid(tTag, "Seller", t.seller);
