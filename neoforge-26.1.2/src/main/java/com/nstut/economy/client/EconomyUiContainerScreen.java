...
public abstract class EconomyUiContainerScreen<M extends AbstractContainerMenu> extends UiContainerScreen<M> {
    ...
    protected EconomyUiContainerScreen(M menu, Inventory inventory, Component title, int imageWidth, int imageHeight) {
    ...
    protected int shellWidth() { return getImageWidth(); }

    protected int shellHeight() { return getImageHeight(); }

    protected void renderBaseShell(GuiGraphicsExtractor g) {
        if (!shellRenderGuard.enter()) return;
        try {
            // AbstractContainerScreen.extractContents(...) invokes OpenUI's
            // renderBackgroundLayer hook, so invoking it here would recurse
            // through OpenUI's renderBackgroundLayer until the stack overflows.
            extractTransparentBackground(g);
            ColorScheme c = colors();
            int radius = currentUiTheme().cardTheme().radius();
            UiRender.surface(g, leftPos, topPos, shellWidth(), shellHeight(),
